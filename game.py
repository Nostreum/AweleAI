...
def getCopieJeu(jeu,computeCoupsValides=True):
    ...
    lc=[getCopieCoup(x) for x in getCoupsJoues(jeu)]
    if(computeCoupsValides):
        lv=[getCopieCoup(x) for x in getCoupsValides(jeu)]
    else:
        if(jeu[2]==None):
            lv=None
        else:
            lv=[getCopieCoup(x) for x in jeu[2]]
...
def finJeu(jeu):
    """ jeu -> bool
        Retourne vrai si le joueur a qui c'est le tour de jouer n'a aucun coup valide a jouer dans jeu
    """
    return len(getCoupsValides(jeu))==0
